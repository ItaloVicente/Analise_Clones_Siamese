...
def write_in_csv(type_revisions, reviews):
    with open("count_new_categoration/" + PROJECT + "(type_revisions)" + ".csv", "a", newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow([type_revisions, reviews])
def write_in_csv_unica(type_clone, reviews, blocks):
    with open("count_new_categoration/" + PROJECT + "(type_unique)" + ".csv", "a", newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow([type_clone, reviews, blocks])
def write_in_csv_multipla(type_clone, reviews, blocks):
    with open("count_new_categoration/" + PROJECT + "(type_multiple)" + ".csv", "a", newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow([type_clone, reviews, blocks])
...
if not os.path.exists("count_new_categoration"):
    os.mkdir("count_new_categoration")

if not os.path.isfile("count_new_categoration/" + PROJECT + "(type_revisions)" + ".csv"):
    with open("count_new_categoration/" + PROJECT + "(type_revisions)" + ".csv", "w", newline='') as csv_file:
        writer = csv.writer(csv_file)
...
if not os.path.isfile("count_new_categoration/" + PROJECT + "(type_unique)" + ".csv"):
    with open("count_new_categoration/" + PROJECT + "(type_unique)" + ".csv", "w", newline='') as csv_file:
        writer = csv.writer(csv_file)
        #apareceu no comeco, meio ou fim, ou apareceu e sumiu, tipo pode ser qualquer um dos 3, inclusive com intersecao, ou nenhum dos 3
        writer.writerow(
            ["type_clone", "reviews", "blocks"])

if not os.path.isfile("count_new_categoration/" + PROJECT + "(type_multiple)" + ".csv"):
    with open("count_new_categoration/" + PROJECT + "(type_multiple)" + ".csv", "w", newline='') as csv_file:
        writer = csv.writer(csv_file)
        #apareceu no comeco, meio ou fim, ou apareceu e sumiu, tipo pode ser qualquer um dos 3, inclusive com intersecao, ou nenhum dos 3
        writer.writerow(
            ["type_clone", "reviews", "blocks"])

with open(caminho_new_categoration, newline='') as arquivo_csv:
...
with open(caminho_new_categoration_unica, newline='') as arquivo_csv:
    leitor_csv = csv.reader(arquivo_csv)
    cabeçalho = next(leitor_csv)
    no_clone_review = 0
...
with open(caminho_new_categoration_multipla, newline='') as arquivo_csv:
    leitor_csv = csv.reader(arquivo_csv)
    cabeçalho = next(leitor_csv)

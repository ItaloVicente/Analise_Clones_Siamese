...
#Elasticsearch precisa estar ativo
#nos arquivos de config do Siamese, eu preciso de dois arquivos, um index e um search, exemplo "index-config.properties" e "search-config.properties"
...
def search_siamese(PROJECT, review, revision):
    caminho_arquivo_config = "/home/gesad/siamese/Siamese/search-config.properties"
    ...
    config['inputFolder'] = "/home/gesad/PycharmProjects/Diff/blocks/" + PROJECT + "/" + review + "_" + revision
    config.write()
    ...
    jar_command = [
        'java',
        '-jar',
        '/home/gesad/siamese/Siamese/siamese-0.0.6-SNAPSHOT.jar',
        '-cf',
        '/home/gesad/siamese/Siamese/search-config.properties'
    ]
...
def index_siamese(PROJECT, review, revision):
    ...
    caminho_arquivo_config = "/home/gesad/siamese/Siamese/index-config.properties"
    nome_index = PROJECT + "_before_" + review + "_" + revision
    ...
    # Modificar os valores desejados
    config['index'] = nome_index
    config['inputFolder'] = "/home/gesad/PycharmProjects/Diff/befores/" + PROJECT + "/before_" + review + "_" + revision
    config.write()
    # Salvar as alterações de volta ao arquivo de configuração
    # Construir o comando para executar o JAR
    jar_command = [
        ...
        '/home/gesad/siamese/Siamese/siamese-0.0.6-SNAPSHOT.jar',
        '-cf',
        '/home/gesad/siamese/Siamese/index-config.properties'
    ]
...
#após uma anáçise é melhor ficar fazendo um por um, pode alterar, mas o algoritmo não salva onde parou o search, só onde estava o index, ou seja, não irá alterar os resultados, mas se o algortimo tiver parado no search, ele irá fazer o search do começo para os befores que estavam na pasta
max_befores = 1
PROJECT = "platform.ui"
...
with open(caminho_do_arquivo, newline='') as arquivo_csv:
    ...
    for linha in leitor_csv:
        if linha[0] != "Review Unable to Download " and linha[0] != "Not Found review: ":
            verificador1=False
            ...
            if int(linha[1]) > (int_review_number_check - 1) and verificador1 == False:
                ...
                if revisions_downloads >= max_befores:
                    ...
                    for nome in sorted(diretorios):
                        ...
                        search_siamese(PROJECT, review_for_index, revision_for_index)
...
if revisions_downloads > 0:
    pasta_desejada = "home/gesad/PycharmProjects/Diff/befores/" + PROJECT
    diretorios = [nome for nome in os.listdir(pasta_desejada) if os.path.isdir(os.path.join(pasta_desejada, nome))]
    for nome in sorted(diretorios):
        L_splitted = nome.split("_")
        review_for_index = L_splitted[1]

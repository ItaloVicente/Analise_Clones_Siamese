...
def analise_revisions_with_blocks(caminho_type_clones):
    ...
    return revisions
def write_in_new_categoration_multipla(review,block, categoria, tipo, duracao , distancia, move_block):
    with open("new categoration/" + PROJECT + "(tipo_multipla).csv", "a", newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(
            [review,block,categoria,tipo,duracao,distancia,move_block])
def write_in_new_categoration_unica(review,block, categoria, move_block):
    with open("new categoration/" + PROJECT + "(tipo_unica).csv", "a", newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(
...
if not os.path.exists("new categoration"):
    os.mkdir("new categoration")

if not os.path.isfile("new categoration/" + PROJECT + "(tipo_unica)" + ".csv"):
    with open("new categoration/" + PROJECT + "(tipo_unica)" + ".csv", "w", newline='') as csv_file:
        writer = csv.writer(csv_file)
...
if not os.path.isfile("new categoration/" + PROJECT + "(tipo_multipla)" + ".csv"):
    with open("new categoration/" + PROJECT + "(tipo_multipla)" + ".csv", "w", newline='') as csv_file:
        writer = csv.writer(csv_file)
        #apareceu no comeco, meio ou fim, ou apareceu e sumiu, tipo pode ser qualquer um dos 3, inclusive com intersecao, ou nenhum dos 3
        writer.writerow(
                ...
                for situation in dict_blocks[block]:
                    ...
                    if (len(dict_blocks[block]) == 1) or (last_clone_block == last and verificador == False):
                        ...
                        if revision >= ini_revision and revision > revision_anterior + 1 and block_chronology == "apa/des":
                            ...
                            verificador2=True
                            write_in_new_categoration_multipla(review,block,"recorrente", block_chronology,None,distancia, block_position)
                    ...
                    for situation in dict_blocks[block]:
                        ...
                        if arquive_diff == 0:
                            if block_position == "arquivo diferente":
                                ...
                                block_position = "mesmo arquivo"
                    write_in_new_categoration_unica(review,block, linha[2], block_position)
            else:
                write_in_new_categoration_unica(review, None, linha[2], None)
...
PROJECT="platform.ui"
#min linhas seguidas adicionadas que desejo considerar como metodo
min_line = 6
...
if os.path.exists("blocks") == False:
    os.mkdir("blocks")

if os.path.exists("blocks/" + PROJECT) == False:
    os.mkdir("blocks/"+PROJECT)

def write_in_java(methods,number, review, revision, arquivo):
    for linha in methods:
        try:
            java = open("blocks/" + PROJECT + "/" + review + "_" + revision + "/"+ arquivo + "_" + str(number) + ".java", "a", encoding="utf-8")
            if len(linha)==1:
                string = linha[0]
                java.write(string[1:] + "\n")
            elif len(linha)>1:
